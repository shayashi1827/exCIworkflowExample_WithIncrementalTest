classdef modelAdvisorAction
    ...
    methods(Access = public)
        function obj = modelAdvisorAction(modelName)
            ...
            if ~exist(fullfile(parentFolder, 'verify', 'guidelinecheck'), 'dir')
                mkdir([parentFolder, '\verify', '\guidelinecheck']);
            end
            obj.rptPath = fullfile(obj.prjRootFolder, 'Design', obj.modelName, 'pipeline', 'analyze', 'verify', 'guidelinecheck');
        ...
        function obj = generateReport(obj)           
            ...
            load(fullfile(obj.prjRootFolder,'Code','logsPath.mat'),'path');
